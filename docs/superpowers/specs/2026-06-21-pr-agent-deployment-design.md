...
- **E — Secrets.** Broker Worker (`secrets-broker/src/index.ts`): secrets = `DEEPSEEK_API_KEY, GITHUB_APP_PRIVATE_KEY, GITHUB_APP_ID, GITHUB_WEBHOOK_SECRET, TUNNEL_CRED`. `fetch-secrets.sh` creates podman secrets (`pr-agent-*`); map to env targets in quadlet. `bootstrap-secrets.sh` fallback. wrangler.toml + vitest (stubs in vitest config, NOT `[vars]`). **Amended (see DECISIONS-LOG, secrets-broker-consolidation):** the broker is the *shared* `secrets.tjw.dev` Worker, **path-scoped per namespace** (`GET /secrets/pr-agent`), not a separate `pr-agent-secrets.tjw.dev`; toolchain pinned to wrangler 4 (Secrets Store GA). The consumer reads `$SECRETS_DOMAIN/secrets/$SECRETS_NS`.
